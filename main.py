...
def add_record():
    ...
    if not name or not email or not contact or not gender or not DOB or not stream:
        mb.showerror('Error!', "Please fill all the empty fields")
    else:
        try:
            connector.execute(
            ...
            connector.commit()
            mb.showinfo('Record added', f"The info of {name} was successfully added")
            reset_fields()
            display_records()
        except:
...
def remove_record():
    if not tree.selection():
        ...
        mb.showinfo('Done', 'The selected record  was successfully deleted.')
...
# Title in window
Label(main, text="College students' contact info", font=headlabelfont, bg='steelblue').pack(side=TOP, fill=X)
...
# Placing the info insertion
Label(left_frame, text="Full Name", font=labelfont, bg=lf_bg).place(relx=0.375, rely=0.05)
...
Label(left_frame, text="Faculty", font=labelfont, bg=lf_bg).place(relx=0.3, rely=0.7)
...
Button(left_frame, text='Save info', font=labelfont, command=add_record, width=14).place(relx=0.025, rely=0.85)
...
Label(right_frame, text='Students Records', font=headlabelfont, bg='cornflowerblue', fg='black').pack(side=TOP, fill=X)

tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                    columns=('Student ID', "Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "Faculty"))
...
tree.heading('Date of Birth', text='Birthday', anchor=CENTER)
